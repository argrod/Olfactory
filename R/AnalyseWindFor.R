...
# remove WindDat at speeds under 15kph
WindDat <- WindDat[WindDat$spTrav > 15,]
WindUhr <- WindDat[which(WindDat$timeTo < (3600)),]
...
gamtst <- gam(absRelHead ~ s(distTo) + s(WSpeed) + s(domFreq) + tripL + OutHm + s(yrID, bs="re") + s(seq,bs="re"),
    data = WindUhr, method = "REML", family = gaussian())
