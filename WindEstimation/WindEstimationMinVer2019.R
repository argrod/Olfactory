...
if(Sys.info()['sysname'] == "Darwin"){
	outfile<-paste("/Volumes/GoogleDrive/My Drive/PhD/Data/2019Shearwater/WindEst/MinDat/", sep=",")
	fileloc <- "/Volumes/GoogleDrive/My Drive/PhD/Data/2019Shearwater/AxyTrek/"
} else {
	outfile<-paste("F:/UTokyoDrive/PhD/Data/2019Shearwater/WindEst/MinDat/", sep=",")
...
for(a in 1:length(files)){
	AxDatFull <- read.delim(paste(fileloc,files[a], sep = ''), sep = '\t', header = F)
	timepoint <- as.POSIXct(as.character(AxDatFull[,1]), format = '%Y/%m/%d,%H:%M:%OS', tz = "") + (9*3600)
    ...
    timepoint <- as.POSIXct(as.character(AxDat[,1]), format = '%Y/%m/%d,%H:%M:%OS') + (9*3600)
